...
function resolveSupabaseConfig() {
  ...
  if (typeof window !== 'undefined') {
    ...
    // Se for localhost ou preview da Vercel (ex: app-git-*, *-projects.vercel.app, preview URLs)
    const isLocal = hostname === 'localhost' || hostname === '127.0.0.1';
    const isVercelPreview = hostname.includes('vercel.app') && !hostname.startsWith('ubtservicos.vercel.app');
    const isDevHomologHost = hostname.includes('dev') || hostname.includes('homolog') || hostname.includes('git') || hostname.includes('preview');
